...
type txMetricDetailTime struct {
	...
	Broker1CommitTimestampSplit, Broker2CommitTimestampSplit time.Time
}
...
func (ttd *TestTxDetail) UpdateMeasureRecord(b *message.BlockInfoMsg) {
	...
	for _, b1tx := range b.Broker1Txs {
		if _, ok := ttd.txHash2DetailTime[string(b1tx.RootTxHash)]; !ok {
			ttd.txHash2DetailTime[string(b1tx.RootTxHash)] = &txMetricDetailTime{}
			ttd.txHash2DetailTime[string(b1tx.RootTxHash)].TxProposeTimestamp = b1tx.Time
			ttd.txHash2DetailTime[string(b1tx.RootTxHash)].BlockProposeTimestamp = b.ProposeTime
			ttd.txHash2DetailTime[string(b1tx.RootTxHash)].Broker1CommitTimestamp = b.CommitTime
		} else {
			if ttd.txHash2DetailTime[string(b1tx.RootTxHash)].TxProposeTimestamp.After(b1tx.Time) {
				ttd.txHash2DetailTime[string(b1tx.RootTxHash)].TxProposeTimestamp = b1tx.Time // 存在，取更早的时间
				ttd.txHash2DetailTime[string(b1tx.RootTxHash)].BlockProposeTimestamp = b.ProposeTime
				ttd.txHash2DetailTime[string(b1tx.RootTxHash)].Broker1CommitTimestampSplit = ttd.txHash2DetailTime[string(b1tx.RootTxHash)].Broker1CommitTimestamp
				ttd.txHash2DetailTime[string(b1tx.RootTxHash)].Broker1CommitTimestamp = b.CommitTime
			} else {
				ttd.txHash2DetailTime[string(b1tx.RootTxHash)].Broker1CommitTimestampSplit = b.CommitTime
			}
		}
	}
	for _, b2tx := range b.Broker2Txs {
		if _, ok := ttd.txHash2DetailTime[string(b2tx.RootTxHash)]; !ok {
			ttd.txHash2DetailTime[string(b2tx.RootTxHash)] = &txMetricDetailTime{}
		}
		ttd.txHash2DetailTime[string(b2tx.RootTxHash)].Broker2CommitTimestamp = b.CommitTime
		ttd.txHash2DetailTime[string(b2tx.RootTxHash)].TxCommitTimestamp = b.CommitTime
	}
}
...
func (ttd *TestTxDetail) writeToCSV() {
	...
	measureName := []string{
		...
		"Broker1.5 Split-Tx commit timestamp (not a broker tx -> nil)",
	...
	for key, val := range ttd.txHash2DetailTime {
		csvLine := []string{
			...
			timestampToString(val.Broker1CommitTimestampSplit),
