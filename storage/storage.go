...
func NewStorage(cc *params.ChainConfig) *Storage {
	...
	s := &Storage{
		dbFilePath:            params.DatabaseWrite_path + "chainDB/S" + strconv.FormatUint(cc.ShardID, 10) + "_" + strconv.FormatUint(cc.NodeID, 10) + "_database",
		blockBucket:           "block",
...
func (s *Storage) UpdateNewestBlock(newestbhash []byte) {
	...
	fmt.Println("Storage.UpdateNewestBlock: The newest block is updated")
}
...
func (s *Storage) AddBlock(b *core.Block) {
	...
	fmt.Println("Storage.AddBlock: Block is added")
}
