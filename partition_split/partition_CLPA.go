...
func (cs *CLPAState) FLPA_Partition() (map[string]uint64, int) {
	cs.ComputeEdges2Shard()
	fmt.Println(cs.CrossShardEdgeNum)
	res := make(map[string]uint64)
	...
	for iter := 0; iter < cs.MaxIterations; iter += 1 { // 第一层循环控制算法次数，constraint
		...
		for v := range QueMap {
			...
			if vNowShard != max_scoreShard && cs.VertexsNumInShard[vNowShard] > 1 {
				...
				for _, u := range cs.NetGraph.EdgeSet[v] {
					_, ok := QueMap[u]
					if cs.PartitionMap[u] != max_scoreShard && ok {
						QueMap[u] = true
					}
				}
			}
...
func (cs *CLPAState) GetNewBridgeSimple(ABridgesNum int, init bool) *core.SingleBridge {
...
func (cs *CLPAState) ABSplit(addNum int, init bool) *core.SingleBridge {
	cs.ComputeEdges2Shard()
